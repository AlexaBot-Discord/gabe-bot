...
module.exports = function (client, options) {
	...
	function play(msg, suffix) {
		...
		if (msg.member.voiceChannel === undefined) return msg.channel.send('<:wonderExclamation:332685258623287296> | **Please, join a voice channel.**');
		// Make sure the suffix exists.
		if (!suffix) return msg.channel.send('<:wonderExclamation:332685258623287296> |  **No video specified!**');
		...
		if (queue.length >= MAX_QUEUE_SIZE) {
			return msg.channel.send('<:wonderExclamation:332685258623287296> |  **Maximum queue size reached!**');
		}

		// Get the video information.
		msg.channel.send('<:wonderSpeech:332686378699849729> |  **Searching...**').then(response => {
			...
			YoutubeDL.getInfo(searchstring, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
				...
				if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
					return response.edit('<:wonderExclamation:332685258623287296> |  **Invalid video!**');
				}

				info.requester = msg.author.id;

				// Queue the video.
				response.edit('<:wonderMusicalNote:341414772476084244> |  **Queued:**  *' + info.title + '*').then(() => {
					queue.push(info);
	...
	function skip(msg, suffix) {
		...
		if (voiceConnection === null) return msg.channel.send('<:wonderSpeech:332686378699849729> |  **No music being played.**');

		// Get the queue.
		const queue = getQueue(msg.guild.id);

		if (!canSkip(msg.member, queue)) return msg.channel.send('<:wonderExclamation:332685258623287296> |  **You cannot skip this as you didn\'t queue it.**').then((response) => {
		...
		msg.channel.send('<:wonderOk:332914524166422530> |  **Skipped** *' + toSkip + '* **!**');
	}
	...
	function queue(msg, suffix) {
		...
		msg.channel.send('<:wonderSpeech:332686378699849729> |  **Queue (' + queueStatus + '):**\n*' + text + '*');
	}
	...
	function pause(msg, suffix) {
		...
		if (!isAdmin(msg.member))
			return msg.channel.send('<:wonderCross:333375581859086337> |  **You are not authorized to use this.**');

		// Pause.
		msg.channel.send('<:wonderOk:332914524166422530> |  **Playback paused.**');
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function leave(msg, suffix) {
		if (isAdmin(msg.member)) {
			const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
			if (voiceConnection === null) return msg.channel.send('<:wonderExclamation:332685258623287296> |  **I\'m not in any channel.**');
			// Clear the queue.
			const queue = getQueue(msg.guild.id);
			...
			msg.channel.send('<:wonderCross:333375581859086337> |  **You don\'t have permission to use that command!**');
		}
	}
	 ...
	 * @param {string} suffix - Command suffix.
	 */
	function stop(msg, suffix) {
		if (isAdmin(msg.member)) {
			const queue = getQueue(msg.guild.id);

			queue.splice(0, queue.length);
			msg.channel.send('<:wonderOk:332914524166422530> |  **Queue cleared!**');
		} else {
			msg.channel.send('<:wonderCross:333375581859086337> |  **You don\'t have permission to use that command!**');
		}
	}
	...
	function resume(msg, suffix) {
		...
		// Resume.
		msg.channel.send('<:wonderOk:332914524166422530> |  **Playback resumed.**');
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function volume(msg, suffix) {
		...
		if (suffix > 200 || suffix < 0) return msg.channel.send('<:wonderWarning:333376928788512768> |  **Volume out of range!**').then((response) => {
			response.delete(5000);
		});

		msg.channel.send("<:wonderOk:332914524166422530> |  **Volume set to " + suffix + '**');
	...
	function executeQueue(msg, queue) {
		...
		if (queue.length === 0) {
			msg.channel.send('<:wonderSpeech:332686378699849729> |  **Playback finished.**');
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			msg.channel.send('<:wonderMusicalNote:341414772476084244> |  **Now Playing:** *' + video.title + '*').then(() => {
